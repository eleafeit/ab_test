...
## Recent buyers versus aged customers
```{r, echo=FALSE}
# aggregate(cbind(open, click, purch) ~ group + (last_purch < 60), 
#          data=d, FUN=mean)
```
```{r}
d %>% group_by((last_purch < 60), group) %>% summarize(mean(open), mean(click), mean(purch))
```

- The email seems to produce a stronger effect on purchases for more recent buyers


## Is email effective for recent buyers? 
```{r,  warning=FALSE}
...
1. Subgroups will vary in how much they engage in behaviors
    - Recent buyers tend to have higher average purchases after the email
...
summary(m2)$coef
```
Aged customers in the control group purchased on average \$5.55 in the 30-days after the email was sent. Recent customers in the control group purchased an additional \$13.55. The average effect of the email was \$6.44. 
...
```{r}
new_cust <- data.frame(past_purch=rep(38.12,2), last_purch=rep(19,2), visits=rep(3,2))
...
```{r}
new_cust <- data.frame(past_purch=rep(127.88,2), last_purch=rep(19,2), visits=rep(40,2))
(pred <- predict(m4, cbind(email=c(TRUE, FALSE), new_cust)))
(lift <- pred[1] - pred[2])
```
...
treat <- d$email
response <- d$purch
baseline <- d[, c("last_purch", "past_purch", "visits", "chard", "sav_blanc", "syrah", "cab")]
...
```{r}
trans_gray <- rgb(0.1, 0.1, 0.1, alpha=0.1)
plot(d$last_purch, predict(cf)$predictions, cex=0.5, col=trans_gray,
