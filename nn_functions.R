...
# old code had a few features that are not maintained -----
# alternative decisionrule
# binomial approximation
# profit_nn_sim <- function(n, N, s1, mu1, sigma1, s2=s1, mu2=mu1, sigma2=sigma1, 
#                           R=1000, pick_winner=FALSE, draws=FALSE, binom_approx=FALSE) {
#   # Function to compute profit by numeric simulation (for testing)
#   # R is the number of simulations
#   # n=c(n1, n2) is a vector of sample sizes
#   # N is the size of the deployment population
#   # s1 and s2 are the (known) std dev of the outcome
#   # mu1 and mu2 are the mean of the prior on the mean response 
#   # sigma1 and sigma2 is the std dev of the prior on the mean response 
#   # binom_approx is a switch for simulating from the approximate binomial
#   draw_mu1 <- rnorm(R, mean=mu1, sd=sigma1)
#   draw_mu2 <- rnorm(R, mean=mu2, sd=sigma2)
#   if (binom_approx) {  # truncate the distribution 
#     draw_mu1[draw_mu1 < 0] <- 1e-6
#     draw_mu2[draw_mu2 < 0] <- 1e-6
#     draw_mu1[draw_mu1 > 1] <- 1 - 1e-6
#     draw_mu1[draw_mu2 > 1] <- 1 - 1e-6
#   }
#   y1 <- y2 <- matrix(NA, nrow=R, ncol=N) # potential outcomes
#   for (r in 1:R) {
#     if (binom_approx) {
#       y1[r,] <- rnorm(N, mean=draw_mu1[r], sd=sqrt(draw_mu1[r]*(1-draw_mu1[r])))
#       y2[r,] <- rnorm(N, mean=draw_mu2[r], sd=sqrt(draw_mu2[r]*(1-draw_mu2[r])))
#     } else {
#       y1[r,] <- rnorm(N, mean=draw_mu1[r], sd=s1)
#       y2[r,] <- rnorm(N, mean=draw_mu2[r], sd=s2)
#     }
#   }
#   profit_test <- rowSums(y1[,1:n[1]]) + rowSums(y2[,n[1] + 1:n[2]])
#   if (pick_winner) {
#     deploy <- 2 - (rowMeans(y1[,1:n[1]]) > rowMeans(y2[,n[1] + 1:n[2]])) # assume no ties
#   } else {
#     sigma1_post <- ( 1/sigma1^2 + n[1]/s1^2 )^-1  # this is a variance
#     sigma2_post <- ( 1/sigma2^2 + n[2]/s2^2 )^-1
#     mu1_post <- sigma1_post * (mu1/sigma1^2 + rowSums(y1[,1:n[1]])/s1^2)
#     mu2_post <- sigma2_post * (mu2/sigma2^2 + rowSums(y2[,n[1] + 1:n[2]])/s2^2)
#     deploy <- 2 - (mu1_post > mu2_post)
#   }
#   profit_deploy <- (2 - deploy) * rowSums(y1[,(n[1] + n[2] + 1):N]) + 
#                    (deploy - 1) * rowSums(y2[,(n[2] + n[2] + 1):N])
#   rand <- 0.5 * draw_mu1 + 0.5 * draw_mu2
#   best <- 2 - (draw_mu1 > draw_mu2)
#   perfect <- (2 - best) * rowSums(y1) + (best - 1) * rowSums(y2)
#   gain <- ((profit_test + profit_deploy)/N - rand) / (perfect/N - rand)
#   error <- !(deploy == best)
#   hist(profit_test + profit_deploy, main="Distribution of Profit", xlab="Profit", freq=FALSE)
#   d <-  data.frame(profit = (profit_test + profit_deploy)/N, 
#                    profit_test = profit_test, 
#                    profit_deploy = profit_deploy, 
#                    profit_rand = rand/N, 
#                    profit_perfect = perfect/N, 
#                    profit_gain = gain, 
#                    deploy = deploy, 
#                    best = best, 
#                    error = error, 
#                    tie=0)
#   s <- data.frame(n1 = n[1], n2 = n[2], 
#                   profit = mean(profit_test + profit_deploy)/N, 
#                   profit_test = mean(profit_test),
#                   profit_deploy = mean(profit_deploy), 
#                   profit_rand = mean(rand), 
#                   profit_perfect = mean(perfect)/N,
#                   profit_gain = mean(gain),
#                   error_rate = mean(error),
#                   deploy_1_rate = mean(2-deploy),
#                   tie_rate=0)
#   if (draws) {out <- list(summary = s, draws = d)} else {out <- s}
#   out
# }
